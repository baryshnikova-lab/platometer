...
class Platometer:
    ...
    def __init__(self, path_to_image_file,
                 plate_format=np.array([32, 48]), verbose=True):
    ...
    def gray_and_trim(self):
        ...
        # First fit (only on the middle of the plate, to get robust estimates
        # of parameters)
        ...
        # Define the expected size of a window containing the expected number
        # of rows or columns
        w_row = np.ceil(self.fit_row['period'] * self.n_rows).astype(int)
        w_col = np.ceil(self.fit_col['period'] * self.n_cols).astype(int)

        # Find the boundaries of the plate (when plate is not centered within
        # the image)
        ...
        row_negative_peaks_px = detect_peaks(
            row_avg_fit,
            mpd=self.fit_row['period'] / 2,
            edge='both',
            kpsh=True,
            valley=True,
            show=False)
        col_negative_peaks_px = detect_peaks(
            col_avg_fit,
            mpd=self.fit_col['period'] / 2,
            edge='both',
            kpsh=True,
            valley=True,
            show=False)

        best_w_row_mid_pxl = row_negative_peaks_px[np.argmin(
            np.abs(best_w_row_mid_pxl - row_negative_peaks_px))]
        best_w_col_mid_pxl = col_negative_peaks_px[np.argmin(
            np.abs(best_w_col_mid_pxl - col_negative_peaks_px))]
    ...
    def detect_colonies(self):
        ...
        row_left_side = np.sort(
            colony_distance_from_center[colony_distance_from_center > 0])[0:mid_row]
        colony_row_pxl = np.sort(
            self.im_center[0] - np.concatenate((row_left_side,
        ...
        col_left_side = np.sort(
            colony_distance_from_center[colony_distance_from_center > 0])[0:mid_col]
        colony_col_pxl = np.sort(
            self.im_center[1] - np.concatenate((col_left_side,
    ...
    def measure_colony_sizes(self):
        ...
        self.im_foreground = estimate_foreground_by_adaptive_thresholding(
            self.im_gray_trimmed, block_size=self.fit_row['period'] * 5)
        ...
        # Identify touching colonies (if any) and separate them using watershed
        # segmentation
        ...
        data = {'col_pxl': colony_col_pxl_2d,
                ...
                'label': self.im_objects[colony_row_pxl_2d,
                                         colony_col_pxl_2d].astype(int)}
    ...
    def process(self):
        ...
        try:
            ...
            print(
                'RuntimeError with processing %s. Moving on...' %
                self.path_to_image_file)
...
def estimate_foreground_by_adaptive_thresholding(
        im_gray_trimmed, block_size=45):
...
def process_folder(folder_name):
    ...
    jpg_files = [
        os.path.join(
            folder_name,
            f) for f in os.listdir(folder_name) if os.path.isfile(
            os.path.join(
                folder_name,
                f)) and f.lower().endswith('jpg') and os.path.getsize(
                    os.path.join(
                        folder_name,
                        f)) > 0]
...
def process_and_save_chunk(
    ...
    if n_processes > 1:
        ...
        for res in pool.map_async(
                run_platometer_batch,
                this_jpg_files_dict).get():
            if res.shape[0] > 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'path_to_image_folder_list',
        metavar='path_to_image_folder_list',
        type=str,
        help='Path to the file containing the list of image folders to process')
    parser.add_argument(
        '--plate_format',
        metavar='plate_format',
        type=int,
        nargs=2,
        default=[
            32,
    ...
    for folder in folders[0]:
        ...
        # Break the list into smaller chunks of 100 images and process the
        # chunks sequentially
        ...
        for n_chunk, ix_chunk in enumerate(chunk_starts):
            ...
            ix_chunk_stop = np.min(
                [ix_chunk + CHUNK_SIZE - 1, nr_jpg_files - 1])

            print('Chunk %d of %d, start %d, stop %d' %
                  (n_chunk, len(chunk_starts), ix_chunk_start, ix_chunk_stop))
