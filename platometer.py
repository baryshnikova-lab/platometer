...
class Platometer:
    ...
    def test(self):
        ...
        n_rows = np.sum(data_test.groupby('row')['size'].count() > self.n_cols/2)
        n_cols = np.sum(data_test.groupby('col')['size'].count() > self.n_rows/2)
    ...
    def show_plate(self, ax=None, **kwargs):
        ...
        if 'row' in kwargs:
            ...
            ax.imshow(img[y_pxl + height:y_pxl, x_pxl:x_pxl + width], **kwargs2)
...
#         h = h*3
        ...
        rect = patches.Rectangle((x_pxl, y_pxl), width, height,
                                 linewidth=2, edgecolor=ecolor, facecolor='none')
    ...
    for folder in folders[0]:
        ...
        for n_chunk, ix_chunk in enumerate(chunk_starts):
            ...
            this_jpg_files_dict = [{'path': jpg_files[i], 'file_id': jpg_files_id[i],
                'plate_format': plate_format} for i in np.arange(ix_chunk_start, ix_chunk_stop+1)]
